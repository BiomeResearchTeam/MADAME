...
class Exp_Proj_MetadataDownload:
    ...
    # WARNING : it needs a list of the AVAILABLE PROJECTS (ProjectManager.getAvailableProjects(listOfProjectIDs))  
        ...
        for accessionID in track(listOfProjectIDs, description="Downloading..."): #--> should change into 'list of accession ids'

            if re.match(PROJECTS_PATTERN, accessionID):
                projectID = accessionID
                study_accession.append(projectID)
                path = os.path.join("Downloads", user_session, projectID)
                Utilities.createDirectory(path)
                self.experimentsMetadataDownload_project(projectID, path)
                self.projectMetadataDownload(projectID, path, print_message = True) 

            else:
                projectID = self.experimentsMetadataDownload_other(accessionID, user_session)
                study_accession.append(projectID)
                path = os.path.join("Downloads", user_session, projectID)
                self.projectMetadataDownload(projectID, path, print_message = False) # to prevent printing multiple times "the file already exists"
        ...
        # Removing duplicates to return a clean list
        study_accession = list(dict.fromkeys(study_accession)) 

        return study_accession

  
    def projectMetadataDownload(self, projectID, path, print_message = True):
    ...
    def experimentsMetadataDownload_project(self, projectID, path):
    ...
    # Download experiments metadata file only if it doesn't exist - other accession code version
        ...
        incoming_df = pd.read_csv(io.StringIO(content.decode('utf-8')), sep='\t')
        ...
        if os.path.isfile(metadata_file):
            ...
            # If not, merge dataframes and save with the same name
                ...
                merged_dataframe = pd.concat([local_df, incoming_df])
    ...
    def updateListOfAccessions(self, user_session, study_accession):
        ...
        print(f'\n{user_session}_listOfAccessionIDs.tsv' + Color.BOLD + Color.GREEN + ' successfully updated' + Color.END)
