        ...
        while True:
            ...
            if data_download_type in ("back"):
                ...
                if merged_experiments.endswith('.tsv'):
                    e_df = pd.read_csv(os.path.join("Downloads", user_session, merged_experiments), delimiter='\t', dtype=str)
                    if 'umbrella_project' in e_df.columns:
                        e_df = pd.read_csv(os.path.join("Downloads", user_session, merged_experiments), delimiter='\t', dtype=str, keep_default_na=False)
